...
//			[[  INPUT FILES  ]]
		final String palettesFilePath = "./resources/palettes.perScreen";
		final String imageFileName = "./resources/Splash Woman Stage.png";
		final String screenReorderFileName = "./resources/screens.reorder";
//			[[  OUTPUT FILES  ]]
...
//			[[  METAMETATILE LISTS]]
//		ArrayList<int[]> MetametatileList = new ArrayList<>();
		...
		try {
			for(int i = 0; i < (level.getWidth() * level.getHeight() / (screenWidthPX * screenHeightPX)); i++) {
				...
				if(new Color(level.getRGB((i%numScreensPerRow)*screenWidthPX, (i/numScreensPerRow)*screenHeightPX), true).getAlpha() > 0) {
					...
					PixelGrabber pixelGrabber = new PixelGrabber(level, x, y, x+screenWidthPX, y+screenHeightPX, currentScreen, 0, level.getWidth());
...
//			[[  PIXELS > TILES AREA  ]]
		...
		for(int screen = 0; screen < palettesPerScreen.size(); screen++) {
			...
			for(int tile = 0; tile < numTilesPerScreen; tile++) {
				...
				//currentTile now has the current tile loaded into it
				//create a treeset that contains all unique items in the currentTile arraylist
				List<Color> palettesOfCurrentTile = Arrays.asList(currentTile).stream().distinct().collect(Collectors.toList());
				...
				for(int palette = 0; palette < 4; palette++) {
					...
					for(int pixelColor = 0; pixelColor < palettesOfCurrentTile.size(); pixelColor++) {
						if(!currentSet.contains(palettesOfCurrentTile.get(pixelColor))) {
							foundANonMatch = true;
						}
					}
					if(!foundANonMatch) {
				...
				if(currentlyUsedPalette == 4) {
					...
					for(int m = 0; m < missingColorList.size(); m++) {
						if(missingColorList.get(m).equals(palettesOfCurrentTile)) {
							exists = true;
						}
					}
					if(!exists) {
						missingColorList.add(palettesOfCurrentTile);
						missingInfoList.add(new int[]{tile, x, y, screen});
					}
				}
...
//			[[  METATILES AREA  ]]		
		...
		for(int screen = 0; screen < ScreensAsTiles.size(); screen++) {
			...
			for(int metatile = 0; metatile < numMetatilesPerScreen; metatile++) {
				...
				for(int palette = 0; palette < currentPalettes.size(); palette++) {
...
//			[[  FILE WRITING SECTION  ]]
		...
		try {
			...
			mtbw.write("ScreensAsMetatiles: \r");
			for (int r = 0; r < ScreensAsMetatiles.size(); r++) {
				mtbw.write("\rscreen_" + r + ": \r");
		...
		try {
			...
			mlbw.flush();
			mlbw.write("\r\rALIGN 256\rLookups_TR: \r");
			for(int t = 0; t < MetatileList.size(); t++) {
				if((t%16) == 0) {
					mlbw.write("HEX  ");
				}
			...
			mlbw.flush();
			mlbw.write("\r\rALIGN 256\rLookups_BL: \r");
			for(int t = 0; t < MetatileList.size(); t++) {
				if((t%16) == 0) {
					mlbw.write("HEX  ");
				}
			...
			mlbw.flush();
			mlbw.write("\r\rALIGN 256\rLookups_BR: \r");
			for(int t = 0; t < MetatileList.size(); t++) {
				if((t%16) == 0) {
					mlbw.write("HEX  ");
				}
			...
			mlbw.flush();
			mlbw.write("\r\rALIGN 256\rattributes: \r");
			for(int t = 0; t < PalettesPerMetatile.size(); t++) {
				mlbw.write("DB  %000000" + String.format("%2s", Integer.toBinaryString(PalettesPerMetatile.get(t))).replace(' ', '0') + "\r");
		...
		try {
			...
			for(ArrayList<int[]> mmt : ListOfMetametatileLists) {
				mlbw.write("METAMETATILE_LOOKUPS_" + listNum + ": \r\rLookups_TL_" + listNum + ": \r");
				...
				mlbw.flush();
				mlbw.write("\r\rALIGN 256\rLookups_TR_" + listNum + ": \r");
				for(int t = 0; t < mmt.size(); t++) {
					if((t%16) == 0) {
						mlbw.write("HEX  ");
					}
				...
				mlbw.flush();
				mlbw.write("\r\rALIGN 256\rLookups_BL_" + listNum + ": \r");
				for(int t = 0; t < mmt.size(); t++) {
					if((t%16) == 0) {
						mlbw.write("HEX  ");
					}
				...
				mlbw.flush();
				mlbw.write("\r\rALIGN 256\rLookups_BR_" + listNum + ": \r");
				for(int t = 0; t < mmt.size(); t++) {
					if((t%16) == 0) {
						mlbw.write("HEX  ");
					}
